...
const MainButton = ({buttonprops}: {buttonprops: ButtonProps}) => {
  ...
  return (
    <Link
      ...
      style={{border: `${!activeButton ? buttonprops.border: buttonprops.hovBorder}`}}
      className={styles.button__wrapper}
      onMouseEnter={() => setActiveButton(true)}
      onMouseLeave={() => setActiveButton(false)}
    >
      <div className={styles.button__content}>
        <span style={{border: `${!activeButton ? buttonprops.border: buttonprops.hovBorder}`}}>{buttonprops.name}</span>
