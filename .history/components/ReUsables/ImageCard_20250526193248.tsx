...
const ImageCard = ({ data }: { data: CardProps }) => {
  return (
    <Link href={data.link} className={styles.report__wrapper} download={data.category==="report" ? true : false}>
      ...
      <div className={styles.report__right}>
        ...
        <span>Posted by <Link href={data.contributor?.link}>{data.contributor?.name}</Link></span>
        </div>
      </div>
    </Link>
  );
