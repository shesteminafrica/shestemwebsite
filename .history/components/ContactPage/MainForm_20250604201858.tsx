...
const MainForm = () => {
  ...
  // Handler to add or remove a service name from activeChecks.
  // The service parameter is expected to have { slug, name }.
  const handleToggleCheck = (service, isActive) => {
    if (isActive) {
      // Add the service name if it isn't already in the list.
      setActiveChecks((prev) =>
        prev.includes(service.name) ? prev : [...prev, service.name]
      );
    } else {
      // Remove the service name if it exists.
      setActiveChecks((prev) =>
  ...
  // For debugging, you can view activeChecks in the console.
  console.log("Active service names:", activeChecks);
